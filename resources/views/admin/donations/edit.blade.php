...
<div class="max-w-3xl space-y-6 pb-12" x-data="{ isSubmitting: false }">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Edit Donation Record #{{ $donation->id }}</h1>
            <p class="text-xs md:text-sm text-slate-600 dark:text-slate-400 mt-1">Update collected unit volume and donation date.</p>
        </div>
    ...
    <div class="bg-white dark:bg-[#0c1427] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 md:p-8 space-y-6">
        <form method="POST" action="{{ route('admin.donations.update', $donation->id) }}" class="space-y-6" @submit="isSubmitting = true">
            @csrf
            @method('PUT')

            <div class="space-y-4">
                <h3 class="text-xs font-extrabold uppercase tracking-wider text-rose-600 dark:text-rose-400 border-b border-slate-100 dark:border-slate-800 pb-2">Record Specifications</h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="quantity" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">Quantity (mL or Units) <span class="text-rose-600">*</span></label>
                        <input id="quantity" type="number" name="quantity" min="1" value="{{ old('quantity', $donation->quantity) }}" required class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none font-mono">
                    </div>
                    ...
                    <div>
                        ...
                        <input id="donation_date" type="date" name="donation_date" value="{{ old('donation_date', $donation->donation_date) }}" required class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <button type="submit" 
                    ...
                    <span x-text="isSubmitting ? 'Updating...' : 'Save Record Changes'"></span>
                </button>
            </div>
        </form>
    </div>
