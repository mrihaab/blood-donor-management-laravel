...
<div class="max-w-4xl space-y-6 pb-12" x-data="{ isSubmitting: false }">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Register New Blood Donor</h1>
            <p class="text-xs md:text-sm text-slate-600 dark:text-slate-400 mt-1">Create a donor account profile with medical ABO/Rh classification and contact details.</p>
        </div>
    ...
    <div class="bg-white dark:bg-[#0c1427] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 md:p-8 space-y-6">
        <form method="POST" action="{{ route('admin.donors.store') }}" class="space-y-6" @submit="isSubmitting = true">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input id="name" type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Tariq Mehmood" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required placeholder="tariq@example.com" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>

                    <div>
                        <label for="password" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">Account Password <span class="text-rose-600">*</span></label>
            ...
            <div class="space-y-4 pt-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input id="date_of_birth" type="date" name="date_of_birth" value="{{ old('date_of_birth') }}" required class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4 pt-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input id="contact_number" type="text" name="contact_number" value="{{ old('contact_number') }}" required placeholder="+92 300 1234567" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <input id="state" type="text" name="state" value="{{ old('state') }}" required placeholder="e.g. Punjab" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="md:col-span-3">
                        ...
                        <input id="address" type="text" name="address" value="{{ old('address') }}" required placeholder="House/Street/Sector details" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                    <div>
                        ...
                        <input id="zip_code" type="text" name="zip_code" value="{{ old('zip_code', '54000') }}" required class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none">
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <button type="submit" 
                    ...
                    <span x-text="isSubmitting ? 'Registering Donor...' : 'Register Donor Account'"></span>
                </button>
            </div>
        </form>
    </div>
