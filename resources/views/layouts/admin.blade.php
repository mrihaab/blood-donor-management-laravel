...
<head>
    ...
    <title>@yield('title', 'Admin Operations Console') — {{ config('app.name', 'LifeBlood System') }}</title>
    ...
    <meta name="apple-mobile-web-app-title" content="LifeBlood Admin">
    ...
    <div class="min-h-screen flex flex-col bg-slate-50 dark:bg-[#090d16]">
        ...
        <div x-show="sidebarOpen" class="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" style="display: none;">
            ...
            <div x-show="sidebarOpen" 
                 ...
                 class="relative flex w-full max-w-xs flex-1 flex-col justify-between bg-white dark:bg-[#0b1329] border-r border-slate-200 dark:border-slate-800 pt-5 pb-8 max-h-screen overflow-y-auto shadow-2xl z-10">
                <div>
                    <div class="flex items-center justify-between px-5 pb-4 border-b border-slate-200 dark:border-slate-800">
                        <div class="flex items-center space-x-3">
                            <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-rose-600 to-red-600 text-white font-black text-lg shadow-lg">🩸</div>
                            <div>
                                <span class="text-base font-extrabold text-slate-900 dark:text-white tracking-tight block">LifeBlood</span>
                                <span class="text-[10px] font-semibold text-rose-600 dark:text-rose-400 uppercase tracking-wider block">Admin Command</span>
                            </div>
                        </div>
                ...
                <div class="p-4 mx-3 my-4 bg-slate-100 dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3">
                    <div class="flex items-center space-x-3">
                        <div class="h-9 w-9 rounded-xl bg-rose-600 text-white flex items-center justify-center font-extrabold text-sm shadow">
                            {{ strtoupper(substr(auth()->user()->name ?? 'A', 0, 2)) }}
                        </div>
                        <div class="overflow-hidden">
                            <p class="text-xs font-bold text-slate-900 dark:text-slate-100 truncate">{{ auth()->user()->name ?? 'Administrator' }}</p>
                            <p class="text-[10px] text-rose-600 dark:text-rose-400 font-semibold truncate">Central Operations</p>
                        </div>
                    </div>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="sticky top-0 z-40 bg-white/90 dark:bg-[#0b1329]/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800/80 px-4 sm:px-8 py-3.5 flex items-center justify-between shadow-sm dark:shadow-xl transition-colors">
                <div class="flex items-center space-x-3 sm:space-x-4">
                    ...
                    <div class="flex items-center space-x-2.5">
                        <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-rose-600 to-red-600 text-white font-black text-sm shadow-md">🩸</div>
                        <div class="flex items-center gap-2">
                            <span class="text-base sm:text-lg font-black text-slate-900 dark:text-white tracking-tight">LifeBlood</span>
                            <span class="hidden sm:inline-block px-2 py-0.5 text-[10px] font-black uppercase tracking-wider bg-rose-100 dark:bg-rose-950 text-rose-700 dark:text-rose-300 rounded-md border border-rose-200 dark:border-rose-800">Admin</span>
                        </div>
                    </div>

                    <!-- Page Subtitle separator -->
                    <div class="hidden md:flex items-center border-l border-slate-200 dark:border-slate-800 pl-3.5 ml-1">
                        <h1 class="text-sm font-bold text-slate-600 dark:text-slate-300">@yield('page_title', 'Central Operations Console')</h1>
                    </div>
                </div>

                <div class="flex items-center space-x-3 sm:space-x-4">
                    <!-- Live Low Inventory Status Pulse Indicator -->
                    @php
                        $invStatus = app(\App\Services\BloodRequestService::class)->getInventoryBalanceStatus();
                    @endphp
                    <a href="{{ route('admin.inventory.index') }}" class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-black border transition shadow-sm {{ $invStatus['badge_class'] }}" title="{{ $invStatus['details'] }}">
                        <span class="h-2 w-2 rounded-full bg-rose-500 animate-pulse"></span>
                        <span>{{ $invStatus['text'] }}</span>
                    </a>
                    ...
                    <div class="relative" x-data="{ 
                        ...
                        <button @click="notifOpen = !notifOpen" class="relative rounded-xl bg-slate-100 dark:bg-slate-900 p-2 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white border border-slate-300 dark:border-slate-800 focus:outline-none transition shadow-sm">
                            ...
                            <template x-if="unreadCount > 0">
                                <span class="absolute -top-1 -right-1 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-rose-600 text-[10px] font-black text-white shadow-lg ring-2 ring-white dark:ring-slate-900" x-text="unreadCount"></span>
                            </template>
                        </button>
                        ...
                        <div x-show="notifOpen" @click.away="notifOpen = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="absolute right-0 mt-2.5 w-88 sm:w-96 rounded-2xl bg-white dark:bg-[#0c1427] p-4 shadow-2xl border border-slate-200 dark:border-slate-800/80 z-50 space-y-3" style="display: none;">
                            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 pb-2.5">
                                <div class="flex items-center gap-2">
                                    <h4 class="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white">Admin Feed</h4>
                                    <span class="text-[10px] font-extrabold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/80 border border-rose-200 dark:border-rose-800 px-2 py-0.5 rounded-full" x-text="unreadCount + ' Unread'"></span>
                                </div>
                                <button @click.prevent="markAllRead()" type="button" class="text-[11px] font-bold text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300 hover:underline flex items-center gap-1">
                            ...
                            <div class="max-h-72 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                                <template x-for="item in notifications" :key="item.id">
                                    <a :href="'/notifications/' + item.id + '/click'" class="block p-3 rounded-xl bg-slate-50 dark:bg-[#111c38] hover:bg-slate-100 dark:hover:bg-[#162447] border border-slate-200/80 dark:border-slate-800 transition group text-left space-y-1">
                                        <div class="flex items-center justify-between">
                                            <span class="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md bg-rose-100 dark:bg-rose-950 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
                                                System Alert
                                            </span>
                                            <span class="text-[10px] text-slate-600 dark:text-slate-400 font-semibold" x-text="item.created_at || 'Just now'"></span>
                                        </div>
                                        <p class="text-xs font-bold text-slate-900 dark:text-slate-100 group-hover:text-rose-600 dark:group-hover:text-rose-400 transition" x-text="item.title"></p>
                                ...
                                <template x-if="notifications.length === 0">
                                    <div class="py-8 text-center space-y-2">
                                        ...
                                        <p class="text-[11px] text-slate-600 dark:text-slate-400">No unread system notifications.</p>
                                    </div>
                                </template>
                            </div>

                            <div class="border-t border-slate-200 dark:border-slate-800/80 pt-2.5 text-center">
                                <a href="{{ route('admin.notifications.index') }}" class="text-xs font-bold text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300 transition">View All Notifications &rarr;</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-8 pb-12">
                @if(session('success'))
