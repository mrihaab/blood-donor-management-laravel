...
@section('title', 'Edit Patient Info')
...
<div class="max-w-4xl space-y-6 pb-12">
    ...
    <x-breadcrumbs :items="[
        ...
        ['label' => $patient->name, 'url' => route('hospital.patients.show', $patient->id)],
        ['label' => 'Edit Record']
    ]" />
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Update Patient Record</h1>
            <p class="text-xs md:text-sm text-slate-500 dark:text-slate-400 mt-1">Modify clinical details for {{ $patient->name }} (MRN: {{ $patient->mrn }}).</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#0c1427] rounded-2xl border border-slate-200 dark:border-slate-800 p-6 md:p-8 shadow-sm transition-colors relative">
        <form method="POST" action="{{ route('hospital.patients.update', $patient->id) }}" x-data="{ isSubmitting: false }" @submit="isSubmitting = true" class="space-y-6">
            @csrf
            @method('PUT')

            <!-- Section 1: Identity & Demographics -->
            <div class="space-y-4">
                <h2 class="text-xs font-extrabold uppercase tracking-wider text-blue-600 dark:text-blue-400">1. Identity & Demographics</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="name" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">Full Patient Name *</label>
                        <input type="text" id="name" name="name" value="{{ old('name', $patient->name) }}" required class="w-full px-4 py-2.5 rounded-xl text-xs md:text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-slate-300 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 focus:outline-none transition shadow-sm">
                        @error('name') <span class="text-xs text-rose-600 dark:text-rose-400 font-semibold mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label for="mrn" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">Medical Record Number (MRN) *</label>
                        <input type="text" id="mrn" name="mrn" value="{{ old('mrn', $patient->mrn) }}" required class="w-full px-4 py-2.5 rounded-xl text-xs md:text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-slate-300 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 focus:outline-none transition shadow-sm font-mono">
                    ...
                    <div>
                        <label for="date_of_birth" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">Date of Birth *</label>
                        <input type="date" id="date_of_birth" name="date_of_birth" value="{{ old('date_of_birth', $patient->date_of_birth ? $patient->date_of_birth->format('Y-m-d') : '') }}" required class="w-full px-4 py-2.5 rounded-xl text-xs md:text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-slate-300 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 focus:outline-none transition shadow-sm font-mono">
                        @error('date_of_birth') <span class="text-xs text-rose-600 dark:text-rose-400 font-semibold mt-1 block">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>

            <!-- Section 2: Blood & Contact -->
            <div class="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <h2 class="text-xs font-extrabold uppercase tracking-wider text-rose-600 dark:text-rose-400">2. Blood & Contact Profile</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="blood_group_id" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">Blood Group</label>
                    ...
                    <div>
                        <label for="contact_number" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">Contact Phone</label>
                        <input type="text" id="contact_number" name="contact_number" value="{{ old('contact_number', $patient->contact_number) }}" class="w-full px-4 py-2.5 rounded-xl text-xs md:text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-slate-300 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 focus:outline-none transition shadow-sm font-mono">
            ...
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                <a href="{{ route('hospital.patients.show', $patient->id) }}" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-xl text-xs transition">Cancel</a>
                <button type="submit" :disabled="isSubmitting" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 active:scale-95 text-white font-extrabold rounded-xl text-xs transition shadow-lg shadow-blue-600/30 disabled:opacity-50 inline-flex items-center gap-1.5">
                    ...
                    <span x-text="isSubmitting ? 'Updating Record...' : 'Update Patient Record'">Update Patient Record</span>
                </button>
            </div>
        </form>
    </div>
